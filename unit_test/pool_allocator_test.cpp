...
TEST_CASE("Testing hydrogen::PooledDeviceAllocator", "[memory][utils][gpu]")
{
    ...
    SECTION("Linear binning test")
    {
        ...
        CHECK(alloc.TotalAllocatedMemory() == (256+3006)*sizeof(float));
        alloc.DeviceFree(ptr);

        alloc.DeviceAllocate(-1, &ptr, sizeof(float) * 4009);
        CHECK(alloc.TotalAllocatedMemory() == (256+3006+4009)*sizeof(float));
        alloc.DeviceFree(ptr);
        CHECK(alloc.TotalAllocatedMemory() == (256+3006)*sizeof(float));
    }
    SECTION("Custom binning test")
    {
        ...
        void *ptr;
        alloc.DeviceAllocate(-1, &ptr, sizeof(float) * 32);
        CHECK(alloc.TotalAllocatedMemory() == 39*sizeof(float));
        alloc.DeviceFree(ptr);

        alloc.DeviceAllocate(-1, &ptr, sizeof(float) * 201);
        CHECK(alloc.TotalAllocatedMemory() == (39+256)*sizeof(float));
        alloc.DeviceFree(ptr);
    }
}
